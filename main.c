    ...
    */
    ...
    for (i = 0; i < 12; i++)
    {
        if (det == a_valid[i])
        {
            valid = 1;
            break;
            //The following code is used for checking if "break;" statement can break "for" loop - The answer is yes
            //printf("%d", a_valid[i]);
        }
    }
    ...
    //convert the plaintext to an array of integers
...
void decryption_Function()
{
    ...
    for (i = 0; i < 12; i++)
    {
        ...
        {
            valid = 1;
            det_inverse = mul_inverse[i];//Fast Inverse by "Cheat" Tables
            //old code for checking the correction of inverse of determinant
            //printf("%d\n", det_inverse);
            break;
        }
    }
    ...
    //convert the ciphertext to an array of integers
